...
import ProductNotification from '../components/ProductNotification.vue';
...
const addCart = (product) => {
  // 檢查購物車是否有資料
  const checkCart = carts.value.find((check) => check.id === product.id);
  if (checkCart) {
    checkCart.quantity++;
  } else {
    carts.value.push({
...
const removeCart = (item) => {
  carts.value = carts.value.filter((check) => check.id !== item.id);
};

// 通知系統功能 (使用 provide/inject)
// 通知的內容
...
<template>
  <div id="app" class="container py-5">
    <div class="row">
      ...
      <!-- 購物車區 -->
      <ProductCart :carts="carts" @remove-cart="removeCart" />
    </div>
